...
def showRestaurants():
    restaurants = \
        session.query(Restaurant).order_by(asc(Restaurant.name))
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized"\
                "to delete this restaurant. Please create your"\
                " own restaurantin order to delete.');}</script>"\
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You are not authorized"\
                ...
                " own restaurant in order to add items.');}</script>"\
                "<body onload='myFunction()''>"
    if request.method == 'POST':
        newItem = MenuItem(
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You are not authorized"\
                ...
                "own restaurant in order to edit items.');}</script>"\
                "<body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
